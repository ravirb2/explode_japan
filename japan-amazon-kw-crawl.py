...
def crawl_datacapture(keywordArr):
	...
	vsponsored_brand = '0'
	driver.get("https://www.amazon.co.jp/s?k="+vkeyword)
	time.sleep(5)
	soup = None
	try:
		html_container = driver.find_element_by_id("search" ).get_attribute('innerHTML')
	...
	if soup is not None:
		productSection =  soup.find("div", {"class" : "sg-col-20-of-24 sg-col-28-of-32 sg-col-16-of-20 sg-col s-right-column sg-col-32-of-36 sg-col-8-of-12 sg-col-12-of-16 sg-col-24-of-28"}, recursive=True)
		if productSection is not None:
			productList = productSection.findAll("div", {"class": "sg-col-4-of-24 sg-col-4-of-12 sg-col-4-of-36 s-result-item sg-col-4-of-28 sg-col-4-of-16 sg-col sg-col-4-of-20 sg-col-4-of-32"}, recursive=True)
		...
		for product in productList:
			...
			## START :: TITLE BLOCK  ##
			title = product.find("h2", {"class": "a-color-base"}, recursive=True)
			...
			## START :: PIRCE BLOCK  ##
			price = product.find("span", {"data-a-color": "price"}, recursive=True)
			if price is not None:
				price1 = price.find('span',{"class":"a-price-whole"},recursive=True)
				if price1 is not None:
					vprice_sp = price1.text.replace(',','')
					vprice_sp = float(vprice_sp)
					vprice_rp = vprice_sp
			
			
			price = product.find("span", {"data-a-color": "secondary"}, recursive=True)
			if price is not None:
				price1 = price.find('span',{"class":"a-price-whole"},recursive=True)
				if price1 is not None:
					vprice_rp = price1.text.replace(',','')
			...
			## END :: PIRCE BLOCK  ##
			## START :: RATING BLOCK  ##
			rate = product.find("a", {"class": "a-popover-trigger a-declarative"}, recursive=True)
			if rate is not None:
				ratings = rate.find("span", {"class": "a-icon-alt"}, recursive=True).string
			else:
				ratings='0'
			if ratings is not None:
				vpdp_rating_value = ratings.replace('5つ星のうち ','')
			else:
				vpdp_rating_value = '0'
				
			#rating_count = product.find("div", {"class": "a-column a-span5 a-span-last"})
			#if rating_count is not None:
			rating_count = product.find("div", {"class": "a-row a-size-small"}, recursive=True)
			if rating_count is not None:
				rc1 = rating_count.find("a",{"class":"a-link-normal"},recursive=True)
				if rc1 is not None:
					vpdp_rating_count = rc1.text.strip()
			else:
				vpdp_rating_count = '0'
			## END :: RATING BLOCK  ##
				
			## START :: IMAGE BLOCK  ##
			image = product.find("img", {"class": "s-image"}, recursive=True)
			if image is not None:
				vpdp_image_url = image["src"]
			else:
				vpdp_image_url = '0'
			vpdp_image_url = (vpdp_image_url[:450] + '..') if len(vpdp_image_url) > 450 else vpdp_image_url
			## START :: IMAGE BLOCK  ##
