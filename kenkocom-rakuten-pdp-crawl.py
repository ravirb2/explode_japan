...
def crawl_datacapture(action,sku):
	...
	#vpdp_page_url = 'http://item.rakuten.co.jp/kenkocom/e460272h' #** CRAWL **
	...
	try:
		...
		if review_div is not None:
			...
			if rd == pre_date:				
				...
				sql1 = "INSERT INTO `kenkocom_crawl_review_info` (`crawl_id`, `web_pid`, `star_rating`, `content_1`, `content_2`, `web_review_ID`, `review_time`, `reviewed_by`,`created_time`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,%s)"
				try:
					cursor.execute(sql1, (
					product_crawl_id, 
					vweb_pid, 
